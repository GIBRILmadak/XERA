...
function buildProfileReturnPath(userId) {
    ...
    const accountType =
        ...
        window.currentUser?.accountType ||
        null;
    const isProAccount = [
        "community",
        "enterprise",
    ...
    if (typeof window.buildProfileUrl === "function") {
        return window.buildProfileUrl(safeUserId || null, accountType);
    }
    if (window.XeraRouter?.buildProfileUrl) {
        return window.XeraRouter.buildProfileUrl(
            safeUserId || null,
            accountType,
        );
    }
    if (window.XeraRouter?.buildHtmlUrl) {
        return window.XeraRouter.buildHtmlUrl(
            isProAccount ? "pagepro" : "profile",
            {
