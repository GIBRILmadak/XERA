...
async function syncDashboardSupportNotification(userId, options = {}) {
    ...
    try {
        ...
        if (!window.creatorDashboardLastSupportNotificationId) {
            window.creatorDashboardLastSupportNotificationId = latestNotification.id;
            return;
        }

        if (window.creatorDashboardLastSupportNotificationId === latestNotification.id) {
            return;
        }
    ...
    const transactionsChannel = supabase
        ...
        .on(
            ...
            (payload) => {
                ...
                if (!['support', 'video_rpm', 'subscription'].includes(next.type)) {
                    return;
                }
            ...
            (payload) => {
                ...
                if (notification.type === 'support' && notification.message) {
                    window.creatorDashboardLastSupportNotificationId =
                        notification.id || window.creatorDashboardLastSupportNotificationId;
...
function getVideoPayoutReferenceDate(payout) {
    return payout?.paid_at || payout?.paidAt || payout?.created_at || payout?.createdAt || payout?.period_month || payout?.periodMonth || null;
}
...
function updateDashboardNavigation(profile) {
    ...
    if (navProfile) {
        ...
        navProfile.title = profile ? `Profil de ${profile.name || 'vous'}` : 'My Trajectory';
    }
...
function renderPayoutSettingsSummary(payoutSettings) {
    ...
    const provider = escapeWalletHtml(
        payoutSettings.providerLabel || payoutSettings.provider || 'Mobile Money',
    );
    ...
    const statusLabel = payoutSettings.status === 'inactive' ? 'Inactif' : 'Actif';
...
function renderWalletOverviewUI(overview) {
    ...
    setText('walletAvailableBalance', formatCurrency(wallet.availableBalance || 0));
    setText('availableBalanceHeader', formatCurrency(wallet.availableBalance || 0));
    setText('walletPendingIncoming', formatCurrency(wallet.pendingIncoming || 0));
    setText('walletPendingWithdrawals', formatCurrency(wallet.pendingWithdrawals || 0));
    setText('walletPaidWithdrawals', formatCurrency(wallet.paidWithdrawals || 0));
    setText(
        'walletMinimumMeta',
    ...
    if (withdrawalAvailable) {
        withdrawalAvailable.value = formatCurrency(wallet.availableBalance || 0);
    }
    ...
    if (withdrawalAmount) {
        ...
        withdrawalAmount.max = availableBalance > 0 ? availableBalance.toFixed(2) : '';
...
async function savePayoutSettings(event) {
    ...
    const accountName = document.getElementById('payoutAccountName')?.value || '';
    const walletNumber = document.getElementById('payoutWalletNumber')?.value || '';
    const countryCode = document.getElementById('payoutCountryCode')?.value || 'CD';
    const notes = document.getElementById('payoutNotes')?.value || '';

    if (!provider || !accountName.trim() || !walletNumber.trim()) {
        showError('Renseignez le fournisseur, le titulaire et le numero Mobile Money.');
        return;
    }

    try {
        setPayoutSettingsSubmitting(true);
        const payload = await fetchDashboardApiJson('/api/monetization/payout-settings', {
            method: 'POST',
            body: JSON.stringify({
                provider,
        ...
        syncPayoutSettingsPanel(window.creatorWalletOverview?.payoutSettings || null);
    }
}
...
async function submitWithdrawalRequest(event) {
    ...
    if (!payoutSettings?.walletNumber || payoutSettings?.status !== 'active') {
        showError('Enregistrez un compte Mobile Money actif avant de demander un retrait.');
        return;
    }
    if (amount < minimumWithdrawal) {
        showError(`Le retrait minimum est de ${formatCurrency(minimumWithdrawal)}.`);
        return;
    }
    ...
    try {
        ...
        showError(error?.message || 'Impossible de créer la demande de retrait.');
    } finally {
        if (submitBtn) {
...
function fillMaxWithdrawal() {
    ...
    const available =
        Number(window.creatorWalletOverview?.wallet?.availableBalance || 0);
...
async function loadRevenueData(userId, period = 'all') {
    try {
        ...
        const [supportResult, videoTransactionsResult, videoPayoutsResult] = await Promise.all([
            supabase
                .from('transactions')
                .select('*')
        ...
        const videoTransactions = (videoTransactionsResult.data || []).filter((tx) =>
            isDateOnOrAfter(tx.created_at, startDate),
        );
        ...
        // Mettre à jour l'UI
        document.getElementById('totalRevenue').textContent = formatCurrency(totalNet);
...
async function processSupport() {
    try {
        ...
        const submitBtn = document.querySelector('#supportModal .btn-primary.btn-full');
