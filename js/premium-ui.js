(function () {
    ...
    function createToast(message, type = 'info') {
        const container = document.getElementById('toast-container') || ensureShell();
    ...
    function createSkeleton(target) {
        ...
        skeleton.innerHTML = '<div class="xera-skeleton__line"></div><div class="xera-skeleton__line short"></div>';
    ...
    async function upgradeToPro() {
        ...
        if (!token) {
            createToast('Please sign in to activate your Pro upgrade.', 'error');
            return false;
        }
        ...
        try {
            ...
            createToast(error.message || 'Unable to activate Pro right now.', 'error');
            return false;
        }
    }
    ...
    function showUpgradeModal() {
        ...
        modal.innerHTML = `
            ...
            <div class="xera-upgrade-modal__panel" role="dialog" aria-modal="true">
                <div class="xera-upgrade-modal__badge">XERA Pro</div>
                <h2>Move faster with XERA Pro</h2>
        ...
        modal.querySelectorAll('[data-close="true"], [data-action="upgrade"]').forEach((element) => {
    ...
    function protectFeature(target) {
        if (!target) return;
        const shouldGate = !window.currentUser?.is_pro && !window.currentUser?.isPro;
    ...
    function initPremiumUi() {
        const lockedTargets = document.querySelectorAll('[data-pro-feature="true"]');
        lockedTargets.forEach((element) => protectFeature(element));

        document.querySelectorAll('[data-loading-skeleton="true"]').forEach((element) => {
