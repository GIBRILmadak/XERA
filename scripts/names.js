...
function generateUniqueName(index) {
    ...
    const sIdx =
        Math.floor(index / (prefixes.length * infixes.length)) %
        suffixes.length;
    const fIdx =
        Math.floor(
            index / (prefixes.length * infixes.length * suffixes.length),
        ) % families.length;
...
// Test de volume
console.log(
    `Combinaisons théoriques : ${data.prefixes.length * data.infixes.length * data.suffixes.length * data.families.length}`,
);
