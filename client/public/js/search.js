...
async function performSearch(query) {
    ...
    try {
        ...
        const { data: content, error: contentError } = await supabase
            .from('content')
            .select('id, user_id, title, description, day_number, page_id, users(name, avatar), professional_pages(name, avatar_url, slug)')
        ...
        const { data: proPages, error: proPagesError } = await supabase
            ...
            .select('id, name, slug, bio, description, industry, avatar_url, hiring_needs')
            .or(`name.ilike.%${searchTerm}%,bio.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,industry.ilike.%${searchTerm}%`)
            .limit(10);
            ...
            html += `
                <div class="search-result-item search-result-item--pro" onclick="openSearchProPage('${escapeHtml(page.slug || '')}');">
                    ...
                    <div class="search-result-info">
                        <div class="search-result-name">${highlightMatch(name, query)} <span class="search-result-badge">OFFICIEL</span></div>
    ...
    if (content.length > 0) {
        ...
        content.forEach(item => {
            ...
            if (item.page_id && item.professional_pages) {
                ...
                authorAvatar = item.professional_pages.avatar_url || 'icons/enterprise.svg';
